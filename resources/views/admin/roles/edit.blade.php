                                    ...
                                    <div class="card-body">
                                        <form class="form form-horizontal" method="post"
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-12 mb-2">
                                                    <div class="form-group">
                                                        <label for="short_label">{{ __('labels.fields.short_label') }}

                                                            <span class="text-danger">*</span>
                                                        </label>
                                                        <input type="text"
                                                               class="form-control @error('short_label') is-invalid @enderror"
                                                               id="short_label" name="short_label"
                                                               placeholder="{{ __('labels.fields.short_label') }}"
                                                               value="{{ old('short_label', $role->short_label) }}" required />
                                                        @error('short_label')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                                ...
                                                <div class="col-12">
                                                    <div class="table-responsive border rounded mt-1">
                                                        ...
                                                        <table class="table table-striped table-borderless">
                                                            ...
                                                            <tbody>
                                                                @foreach (config('permission.permissions-list.admin') as $model => $permissions)
                                                                    <tr>

                                                                        <td>{{ $model }}</td>
                                                                        @foreach ($permissions['capabilities'] as $permission)
                                                                            <td>
                                                                                <div class="custom-control custom-checkbox">
                                                                                    <input type="checkbox"
                                                                                        class="custom-control-input"
                                                                                        id="{{ $permission }}-{{ $model }}"
                                                                                        @if ($role->permissions->contains('name', $permission . '-' . $model)) checked @endif
